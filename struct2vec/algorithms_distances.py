...
def generate_distances_network(workers):
    ...
    pattern = os.path.join(returnPathStruc2vec(), '..', 'pickles', 'weights_distances-layer-*.pickle')
    ...
    pattern = os.path.join(returnPathStruc2vec(), '..', 'pickles', 'graphs-layer-*.pickle')
    for file in glob.glob(pattern):
        os.remove(file)
    with ProcessPoolExecutor(max_workers=1) as executor:
    ...
    t0 = time()
    pattern = os.path.join(returnPathStruc2vec(), '..', 'pickles', 'distances_nets_weights-layer-*.pickle')
    for file in glob.glob(pattern):
        os.remove(file)
    pattern = os.path.join(returnPathStruc2vec(), '..', 'pickles', 'alias_method_j-layer-*.pickle')
    for file in glob.glob(pattern):
        os.remove(file)
    pattern = os.path.join(returnPathStruc2vec(), '..', 'pickles', 'alias_method_q-layer-*.pickle')
    for file in glob.glob(pattern):
        os.remove(file)
